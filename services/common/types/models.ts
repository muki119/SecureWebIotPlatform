 ...
 */
export type ModelDTO<T extends ModelSchema> = Omit<
	T,
	"id" | "createdAt" | "deletedAt" | "updatedAt"
>;

export type MongoModelSchema<T extends ModelSchema> = Omit<T, "id"> & {
	_id: Schema.Types.UUID;
}; // for mongo models we use _id instead of id and its a uuid type
 ...
 */
export interface IPostgresDatabaseModelOperations<T extends ModelSchema> {
	// all database models should implement this basic interface for ease of use and ability to change db if needed
 ...
 */
export type UpdateSet<T extends ModelSchema> = {
	[K in keyof ModelDTO<T>]-?: { field: K & string; value: ModelDTO<T>[K] };
}[keyof ModelDTO<T>];
...
export abstract class BasePostgresModel<T extends ModelSchema> {
	 ...
	 */
	protected async transactionWrap<U>(
		operation: DbOperation<U>,
	 ...
	 */
	protected async createSetValues(
		patch: UpdatePatch<T>,
		startIdx: number = 2,
	): Promise<[string, unknown[]]> {
		const setStringArr: string[] = [];
		const seenSet = new Set<keyof ModelDTO<T>>(); // to keep track of what is already been set in the patch
		...
		for (let i = 0; i < patch.length; i++) {
			...
			if (!change?.field || change.value === undefined) {
				throw new Error(
					`Field and value are required for all changes in update patch`,
				);
			}
			if (!this.updatableFieldsMap.has(change.field)) {
				throw new Error(
					`Invalid field ${change.field} in update patch`,
				);
			}
			if (seenSet.has(change.field)) {
				throw new Error(
					`Duplicate field ${change.field} in update patch`,
				);
			}
			if (
				typeof change.value !==
				this.updatableFieldsMap.get(change.field)
			) {
				throw new Error(
					`Invalid data type for field ${change.field}: expected ${this.updatableFieldsMap.get(change.field)}, got ${typeof change.value}`,
				);
			}
 ...
 *
 */
export abstract class PostgresDatabaseModel<T extends ModelSchema>
	extends BasePostgresModel<T>
	implements IPostgresDatabaseModelOperations<T>
{
 ...
 */
export abstract class PostgresAssociationModel<T extends ModelSchema>
	extends BasePostgresModel<T>
	implements IPostgresAssociationModelOperations<T>
{
	public abstract create(item: ModelDTO<T>): Promise<T>;
...
export abstract class BaseMongoModel<T extends ModelSchema> {
	...
	async createUpdateObject(
		...
		[Partial<Record<keyof ModelDTO<T>, unknown>>, null] | [null, Error]
	> {
		const seenSet = new Set<keyof ModelDTO<T>>();
		const result: Partial<Record<keyof ModelDTO<T>, unknown>> = {};
		for (let i = 0; i < patch.length; i++) {
			const change = patch[i];
			if (!change?.field || change.value === undefined) {
				return [
			...
			if (!this.updatableFieldMap.has(change.field)) {
				return [
					null,
					new Error(`Invalid field ${change.field} in update patch`),
				];
			}
			if (seenSet.has(change.field)) {
				return [
			...
			if (
				typeof change.value !== this.updatableFieldMap.get(change.field)
			) {
				return [
					null,
					new Error(
						`Invalid data type for field ${change.field}: expected ${this.updatableFieldMap.get(change.field)}, got ${typeof change.value}`,
					),
				];
			}
			seenSet.add(change.field);
	...
	async transactionWrap<U>(
		operation: (session: ClientSession) => Promise<U>,
		externalSession?: ClientSession,
	): Promise<U> {
		if (externalSession) {
...
export abstract class MongoDatabaseModel<T extends ModelSchema>
	extends BaseMongoModel<T>
	implements IMongoDatabaseModelOperations<T>
{
...
export abstract class MongoAssociationModel<T extends ModelSchema>
	extends BaseMongoModel<T>
	implements IMongoAssociationModelOperations<T>
{
	public abstract create(
		item: ModelDTO<T>,
		externalSession?: ClientSession,

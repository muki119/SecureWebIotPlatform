 ...
 * @returns 
 */
export function CreateVerifyAccessTokenInstance(logger: Logger, accessTokenPublicKey?: string, audience?: string) { // have to make a factory function because the logger may have different configurations - but will all be a pino instance anyway 
    /**
    ...
    */
    return (token: string): Result<AccessTokenClaims> => {
        ...
        try {
            const ACCESS_TOKEN_PUBLIC_KEY = accessTokenPublicKey || GetPemKey("ACCESS_TOKEN_PUBLIC_KEY_PATH"); //  for verifying access tokens
            ...
            if (error instanceof jwt.JsonWebTokenError) { // if theres any token invalidity like signature is different or the token is malformed
                return [null, new Error(`Invalid Access token: ${error.message}`)];
            }

...
import { UserRoleModelInstance, TransactionModelInstance } from "../models"
export async function GetDomainTransactionsService(userId: string, domainId: string, dateFrom: Date, dateTo?: Date): Promise<Result<any>> { // date-to is optional , jus return at max 100 transaction from datefrom
    try {
        if (!dateFrom) {
            dateFrom = new Date()
        }
        ...
        throw new Error("Error in GetDomainTransactionsService", { cause: error })
    }

...
export class TransactionModel extends BasePostgresModel<ITransactionModel> {
	...
	public async create(
		item: ModelDTO<ITransactionModel>,
		...
		return this.transactionWrap(async (conn) => {
            ...
            `;
				const values = [
					item.initiatorId,
					item.opperationType,
					item.opperationTarget,
					item.targetId,
					item.value,
					item.opperationTimestamp,
					item.domainId,
				];
	...
	public async findByDomainId(
		...
		// date from is the starting point  - should always be earlier than date to
		return this.poolWrap(async (conn) => {
            ...
            `;
				const values: [string, Date] = [domainId, dateFrom];
				if (dateTo) {
				...
				throw new Error("Failed to get transactions by domain id: ", {
					cause: error,
				});
			}
		});

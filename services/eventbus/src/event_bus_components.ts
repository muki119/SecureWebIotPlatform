			...
			options && {
				socket: {
					reconnectStrategy: (retries) => {
						if (retries > 10) {
							throw new Error(
								"Unable to connect to Redis after 10 retries",
							);
						}
				...
				stream.messages.map(async (message) => {
					...
					return handler(message)
						.then(() => {
							...
							if (!message?.id) {
								throw new Error(
									`Message does not have an id for stream "${streamName}"`,
								);
							}
...
class semaphore {
	...
	public acquire(): Promise<void> {
		if (this.count > 0) {
			...
			return new Promise((resolve) => {
				// if theres no slots , make a promise and add to the waiting queue - the calling function will then await this promise
	...
	public wait(): Promise<void> {
		if (this.count === this.maxCount) {
			...
			// if theres still some processing going on then wait for all the slots to be released - done by just adding the wait promise to the waiting queue so the final release will resolve it and allow the wait to continue
			return new Promise((resolve) => {

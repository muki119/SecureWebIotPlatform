...
export default async function CredentialChangeController(
	req: Request,
	res: Response,
	next: NextFunction,
) {
	try {
		...
		const {
			changes,
			password,
		}: { changes: UpdatePatch<IUser>; password?: string } = req.body;
		...
		const [_, err] = await CredentialChangeService(
			changes,
			userId,
			password,
		);
		...
		logger.info(
			{ userId: userId },
			`Credential change successful for user ${userId}}`,
		);
		return res
			.status(200)
			.json({ message: "Changes applied successfully" })
			.end();
	} catch (err) {
		next(err);
	}

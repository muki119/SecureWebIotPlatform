...
export default async function LogoutController(
	...
	try {
		...
		const refreshTokenClaims = refreshToken
			? TokenBundleInstance.VerifyRefreshToken(refreshToken)
			: null; // thing is if its null then its already invald to the system
		if (refreshTokenClaims) {
			const _ = await TokenBundleInstance.BlockToken(
				refreshTokenClaims.jti,
				refreshTokenClaims.exp,
			); // block the token until it expires - this is to prevent reuse of the same refresh token after logout

...
export class TokenBundle {
    ...
    constructor(config: TokenBundleConfig) {
        ...
        this.VerifyAccessToken = CreateVerifyAccessTokenInstance(logger, config.accessTokenPublicKey, config.audience); // create a verify access token function with the logger instance - this way we can log invalid tokens with the correct logger configuration without having to pass the logger around everywhere
     ...
     */
    CreateBundle(userId: string): Tokens & { expiry: Seconds } { // expiry date for the xsrf token to match the refresh token
        const issuer = "LOGIN";
        const accessToken = this.CreateAccessToken(userId, issuer);
        const { token: refreshToken, jti, expiry } = this.CreateRefreshToken(userId);
     ...
     */
    async RefreshTokens(claims: RefreshTokenClaims): Promise<Result<Tokens & { expiry: Seconds }>> { // for token rotation
        try {
     ...
     */
    private createRefreshToken(userId: string, expire: Date, issuer: string): { token: string, jti: string, expiry: Seconds } { // the actual function to generate refresh tokens
        if (!userId) {
        ...
        try {
            ...
            const token = jwt.sign(registeredClaims, this.REFRESH_TOKEN_KEY, { algorithm: 'HS256' });
            return {
     ...
     * @returns the generated refresh token and its jti 
     */
    private CreateRefreshToken(userId: string) { // create a refresh token on login - wrapper for generateRefreshToken
     ...
     */
    private CreateRefreshFromClaims(claims: RefreshTokenClaims) { // create a refresh token from the claims of a previous refresh token - for token rotation-wrapper for generateRefreshToken
    ...
    private CreateAccessToken(userId: string, issuer: string): string {
        ...
        try {
            const registeredClaims = {
                ...
                aud: this.AUDIENCE,
                iss: issuer,
                exp: Math.floor(Date.now() / 1000) + this.ACCESS_TOKEN_EXPIRATION, // access tokens expire in 15 minutes
                iat: Math.floor(Date.now() / 1000), // issued at time
            }
            const token = jwt.sign(registeredClaims, this.ACCESS_TOKEN_PRIVATE_KEY, { algorithm: 'RS256' });
            return token;
        } catch (err) {
     ...
     */
    private CreateXsrfToken(jti: string, expiry: Seconds, issuer: string): string { //good thing about the xsrf token is that it dosent need to be blocked since its tied to a single refresh token and is dead when the refrehs token is dead
        if (!jti) {
        ...
        try {
            ...
            const token = jwt.sign(registeredClaims, this.XSRF_TOKEN_KEY, { algorithm: 'HS256' });
            return token;
        } catch (error) {
    ...
    */
    ...
    public VerifyRefreshToken(token: string): RefreshTokenClaims | null {// null and val will act as truthy
        if (!token) {
            return null; // if theres no token then just treat it as invalid and return null
        }
        try {
            const r = jwt.verify(token, this.REFRESH_TOKEN_KEY, { algorithms: ['HS256'], audience: this.AUDIENCE }) as RefreshTokenClaims;
            return r;
        } catch (error) {
    ...
    public VerifyXsrfToken(token: string, jti: string): boolean { // verify the xsrf token by verifying its signature and checking if the jti in its claims matches the given jti
        if (!token) {
        ...
        try {
            const claims = jwt.verify(token, this.XSRF_TOKEN_KEY, { algorithms: ['HS256'], audience: this.AUDIENCE }) as jwt.JwtPayload;
     ...
     */
    private async IsBlocked(jti: string): Promise<boolean> { // check if a token is blocklisted - used for refresh tokens
        if (!jti) {
    ...
    public async BlockToken(jti: string, exp: Seconds): Promise<void> { // blocklist a token - used for refresh tokens
        ...
        try {
            ...
            await this.blocklistClient.set(jti, "blocked", { expiration: { type: "EX", value: ttl } }); // set the blocklist key to expire at the same time as the token
        } catch (err) {

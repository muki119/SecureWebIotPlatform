 ...
 */
export async function CreateResetToken(userId: string): Promise<ResetToken> { // should also probably check if theres already one for this userid
 ...
 */
export async function VerifyResetToken(token: ResetToken): Promise<string | null> {
    try {

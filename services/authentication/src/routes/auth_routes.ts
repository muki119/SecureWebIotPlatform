...
authRoutes.post("/forgot-password", ForgotPasswordController); // creates a id in redis attached to the account and sends an email with the id as a query parameter to the reset password page, when the user clicks on the link in the email it will take them to the reset password page where they can enter their new password and the id will be used to verify that the request is valid and then the password will be updated in the database and the id will be deleted from redis
authRoutes.post("/reset-password", ResetPasswordController); // only needs password and the id in the redis as a query parameter, the id will be used to verify that the request is valid and then the password will be updated in the database and the id will be deleted from redis
authRoutes.post("/credential-change", CredentialChangeController); // should be dynamic based on what credential is being changed, for example if email is being changed then we need to send a verification email to the new email address and if password is being changed then we need to send a verification email to the old email address

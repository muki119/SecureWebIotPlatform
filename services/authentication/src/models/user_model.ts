...
export interface IUser extends ModelSchema {
	// this application wont utilise a username field i think , should probably be email instead
	id: string;
	forename: string;
	surname: string;
	email: string;
...
export default class UserModel extends PostgresDatabaseModel<IUser> {
	...
	public async findByIdWithoutPassword(
		id: string,
	): Promise<Omit<IUser, "password"> | null> {
		// same as find by id but without the password field for security reasons
		return this.poolWrap(async (conn) => {
			try {
				const query = `
	...
	public async delete(id: string, externalConn?: PoolClient): Promise<void> {
		// soft delete by setting deletedAt field to current timestamp
		return this.transactionWrap(async (conn) => {
			try {
				const query = `
                UPDATE users
	...
	public async existsById(id: string): Promise<boolean> {
		return this.poolWrap(async (conn) => {
            ...
            `;
				...
				throw new Error("Failed to check if user exists by id: ", {
					cause: err,
				});
			}
		});

...
import {
	TokenBundle,
	type TokenBundleConfig,
} from "../src/helpers/token_bundle";
...
describe("TokenBundle Tests", async () => {
	...
	test("TokenBundle should verify access token correctly", async () => {
		const userId = "test-user";
		const bundle = await testTokenBundle.CreateBundle(userId);
		const [claims, err] = testTokenBundle.VerifyAccessToken(
			bundle.accessToken,
		);
		...
		assert.strictEqual(
			claims.sub,
			userId,
			"User ID in claims should match the original user ID",
		);
	});
	...
	test("TokenBundle should verify xsrf token correctly", async () => {
		...
		const claims = testTokenBundle.VerifyRefreshToken(bundle.refreshToken);
		assert.isNotNull(claims, "Claims should not be null");
		const isValidXsrf = testTokenBundle.VerifyXsrfToken(
			bundle.xsrfToken,
			claims.jti,
		);
	...
	test("TokenBundle should blocklist refresh token correctly", async () => {
		...
		const claims = testTokenBundle.VerifyRefreshToken(bundle.refreshToken);
		assert.isNotNull(claims, "Claims should not be null");
		const [_, blockError] = await testTokenBundle.BlockToken(
			claims.jti,
			claims.exp,
		);
		assert.isNull(blockError, "Error should be null for a valid block");
		const [, err] = await testTokenBundle.RefreshTokens(claims);
		assert.isNotNull(err, "Error should not be null for blocklisted token");
		assert.strictEqual(
			err?.message,
			"Token is blocked",
			"Error message should indicate that the token is blocked",
		);
	});
	...
	test("Invalid Access Token should not be verified", async () => {
		...
		const [claims, err] =
			testTokenBundle.VerifyAccessToken(invalidAccessToken);
		assert.isNull(claims, "Claims should be null for invalid access token");
		assert.isNotNull(
			err,
			"Error should not be null for invalid access token",
		);
	});
	test("Invalid Refresh Token should not be verified", async () => {
		...
		assert.isNull(
			claims,
			"Claims should be null for invalid refresh token",
		);
	});
	...
	test("Bad Audience in Token Bundle should not be verified", async () => {
		...
		const differentAudienceTokenBundle = new TokenBundle(
			differentAudienceConfig,
		);
		const [accessTokenClaims, err] =
			differentAudienceTokenBundle.VerifyAccessToken(bundle.accessToken);
		assert.isNull(
			accessTokenClaims,
			"Claims should be null for access token with bad audience",
		);
		assert.isNotNull(
			err,
			"Error should not be null for access token with bad audience",
		);
		const refreshTokenClaims =
			differentAudienceTokenBundle.VerifyRefreshToken(
				bundle.refreshToken,
			);
		assert.isNull(
			refreshTokenClaims,
			"Claims should be null for refresh token with bad audience",
		);
		const isValidXsrf = differentAudienceTokenBundle.VerifyXsrfToken(
			bundle.xsrfToken,
			"test-jti",
		);
		assert.isFalse(isValidXsrf, "XSRF token should be invalid");
	...
	test("Different JTI in XSRF Token should not be verified", async () => {
		...
		assert.isFalse(
			isValidXsrf,
			"XSRF token should be invalid for different JTI",
		);
	});
	...
	test("RefreshTokens should return a new valid token bundle", async () => {
		...
		assert.exists(
			newBundle.accessToken,
			"New access token should be generated",
		);
		assert.exists(
			newBundle.refreshToken,
			"New refresh token should be generated",
		);
		assert.exists(
			newBundle.xsrfToken,
			"New XSRF token should be generated",
		);
	...
	test("RefreshTokens should return tokens with the correct user id", async () => {
		...
		const [newBundle, err] = await testTokenBundle.RefreshTokens(claims);
		assert.isNull(err, "Error should be null for a valid refresh");
		const [newAccessClaims, accessErr] = testTokenBundle.VerifyAccessToken(
			newBundle.accessToken,
		);
		assert.isNull(
			accessErr,
			"Error should be null for a valid access token",
		);
		assert.strictEqual(
			newAccessClaims.sub,
			userId,
			"User ID in new access token claims should match the original user ID",
		);
		const newRefreshClaims = testTokenBundle.VerifyRefreshToken(
			newBundle.refreshToken,
		);
		assert.isNotNull(
			newRefreshClaims,
			"New refresh token claims should not be null",
		);
		assert.strictEqual(
			newRefreshClaims.sub,
			userId,
			"User ID in new refresh token claims should match the original user ID",
		);
	});
	...
	test("RefreshTokens should preserve the original expiry", async () => {
		...
		assert.strictEqual(
			newBundle.expiry,
			claims.exp,
			"Expiry of new refresh token should match the original refresh token expiry",
		);
	});
	...
	test("RefreshTokens should blocklist the old refresh token", async () => {
		...
		const [newBundle, refreshErr] =
			await testTokenBundle.RefreshTokens(claims);
		assert.isNull(
			newBundle,
			"New bundle should be null for a blocklisted token",
		);
		assert.isNotNull(
			refreshErr,
			"Error should not be null for a blocklisted token",
		);
		assert.strictEqual(
			refreshErr?.message,
			"Token is blocked",
	...
	test("RefreshTokens new XSRF token should be tied to the new refresh token", async () => {
		...
		assert.isTrue(
			isValidXsrf,
			"New XSRF token should be tied to the new refresh token's JTI",
		);
		// old xsrf should not be valid for new refresh token
		const isOldXsrfValid = testTokenBundle.VerifyXsrfToken(
			bundle.xsrfToken,
			newRefreshClaims.jti,
		);
		assert.isFalse(
			isOldXsrfValid,
			"Old XSRF token should not be valid for the new refresh token",
		);
	});
	...
	test("BlockToken should return an error for already blocked token", async () => {
		...
		assert.isNull(blockError, "Error should be null for a valid block");
		const [__, secondBlockError] = await testTokenBundle.BlockToken(
			claims.jti,
			claims.exp,
		);
		assert.isNotNull(
			secondBlockError,
			"Error should not be null for an already blocked token",
		);
		assert.strictEqual(
			secondBlockError?.message,
			"Token is blocked",
			"Error message should indicate that the token is blocked",
		);
	});
	...
	test("BlockToken is Atomic", async () => {
		...
		assert.strictEqual(
			successCount,
			1,
			"Only one block operation should succeed",
		);
		assert.strictEqual(
			failureCount,
			1,
			"One block operation should fail due to already blocked token",
		);
	});
});

...
import type { DeviceCapabilities, DeviceTokenClaims, IDevice } from "../types";
...
export function CreateDeviceToken(device: IDevice): string {
	try {
		const payload: Partial<DeviceTokenClaims> = {
			...
			capabilities:
				device.capabilities instanceof Map
					? Object.fromEntries(device.capabilities)
					: (device.capabilities as Record<
							string,
							DeviceCapabilities
						>),
		};
		return jwt.sign(payload, GetEnvString("DEVICE_TOKEN_SECRET_KEY"), {
			algorithm: "HS256",
		});
	} catch (error) {
...
export function VerifyDeviceToken(token: string): Result<DeviceTokenClaims> {
	try {
		const r = jwt.verify(token, GetEnvString("DEVICE_TOKEN_SECRET_KEY"), {
			algorithms: ["HS256"],

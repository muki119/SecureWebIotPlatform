...
export function VerifyPairingCode(code: string, CodeLength: number = CODE_LENGTH): boolean {
    ...
    for (let i = 0; i < code.length; i++) {
        ...
        if (i >= 2 && code[i] === code[i - 1] && code[i] === code[i - 2]) {// if the current char has 3 of the same char in a row then its invalid - max in window of 3 is 2 
            return false
        }
        if (i > 0) {

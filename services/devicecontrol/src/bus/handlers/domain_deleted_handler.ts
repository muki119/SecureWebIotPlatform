...
export async function DomainDeletedHandler(message: EventPayload) {
	try {
		...
		SocketEmitterInstance.to(domainId).emit(
			SOCKET_EVENTS.SERVER_EMITTED.DOMAIN.DELETED,
			{ domainId },
		); // notify connected clients that the domain has been deleted so they can update their UI accordingly
		...
		MqttClientInstance.publishAsync(
			MQTT_TOPICS.SERVER_EMITTED.DOMAIN(domainId).DELETED,
			JSON.stringify({ domainId }),
		); // publish to mqtt in case any devices are still subscribed to the domain topic so they can stop receiving stale domain events
	} catch (error) {
		throw new Error("Failed to process domain deleted event", {
			cause: error,
		});
	}
}

...
import { EventBusInstance, io } from "../config";
	...
	try {
		...
		if (!userPermissions?.canManageDevices) {
			return [
				null,
				new Error(
					"User does not have permissions to update devices in this domain",
				),
			];
		}
		const [updatedDevice, updateErr] = await DeviceModelInstance.update(
			deviceId,
			patch,
		);
		if (updateErr) {
		...
		io.to(device.domainId as string).emit(
			...
			{ device: updatedDevice, domainId: device.domainId },
		); // notify connected clients that a device has been updated
		...
		}); // send the updated device info to the event bus - we can send the whole updated device info or just the changes and the device id and domain id and let the other services decide if they want to fetch the updated device info or not - for now we will send just the changes and the device id and domain id and let the other services decide if they want to fetch the updated device info or not;

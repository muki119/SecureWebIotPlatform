...
export async function SocketRoutes(socket: Socket) {
	try {
		const userId = socket.user?.sub; // jwt payload sub is user id
		if (!userId) {
			socket.disconnect(true);
			return;
		}
		const userDomains =
			await UserRoleModelInstance.getDomainIdsByUserId(userId);
		const rooms = userDomains
			.map((domainId) => domainId.toString())
			.concat(userId); // also join a room for the user id - for sending user specific notifications like role updates, token expiry etc;
		...
		socket.on(
			...
			(data, callback) =>
				DeviceControlUpdateHandler(socket, data, callback),
		...
		socket.disconnect(true);
		logger.error({ error }, "Error in socket routes");
	}
}

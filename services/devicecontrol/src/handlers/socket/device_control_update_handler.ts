...
export async function DeviceControlUpdateHandler(
	socket: Socket,
	data: DeviceControlUpdateData,
	callback: unknown,
) {
	...
	try {
		...
		if (!userId) {
			logger.warn("User ID not found in socket");
			return ack({ code: 401, error: "Unauthorized" });
		}
		...
		if (!userPermissions?.canControlDevices) {
			logger.warn(
				{ userId, domainId },
				"Unauthorized device control attempt:",
			);
			ack({ code: 401, error: "Unauthorized" });
			return;
		}
		const [, updateErr] = await DeviceModelInstance.updateCurrentState(
			deviceId,
			changes.capability,
			changes.value,
		);
		...
		await MqttClientInstance.publishAsync(
			MQTT_TOPICS.SERVER_EMITTED.COMMANDS(deviceId),
			JSON.stringify({
				capability: changes.capability,
				value: changes.value,
			}),
		);
		DeviceTelemetryModelInstance.create(
			deviceId,
			changes.capability,
			changes.value,
		).catch((err) => {
			logger.warn(
				{ err },
				"Failed to write telemetry for control update",
			);
		});
		socket.broadcast
			.to(domainId)
			.emit(SOCKET_EVENTS.SERVER_EMITTED.DEVICE.UPDATED, {
				deviceId,
				domainId,
				changes,
			});

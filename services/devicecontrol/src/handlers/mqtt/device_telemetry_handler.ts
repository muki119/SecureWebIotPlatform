...
export async function HandleDeviceTelemetry(deviceId: string, message: Buffer) {
	...
	try {
		const { capability, value } = JSON.parse(message.toString()) as {
			capability: string;
			value: string | number | boolean;
		};
		DeviceTelemetryModelInstance.create(deviceId, capability, value).catch(
			(err) => {
				logger.warn(
					{ err },
					"Failed to write telemetry for device emitted telemetry",
				);
			},
		);
		const [updatedDevice, err] =
			await DeviceModelInstance.updateCurrentState(
				deviceId,
				capability,
				value,
			);
		...
		io.to(updatedDevice.domainId as string).emit(

...
export class DeviceTelemetryModel {
	// extends nothing because telemetry is only written and read (paginated)- dosent conform to usual crud since its just logs basicallys
	...
	async create(
		...
		try {
			if (
				...
				throw new Error(
					"Invalid input types for deviceId or capability",
				);
			}
			if (
				typeof value !== "string" &&
			...
			throw new Error("Failed to create device telemetry", {
				cause: error,
			});
		}
	}

	private getIntervalBounds(
		queryDate: Date,
		interval: Intervals,
	): [Date, Date] {
		switch (interval) {
	...
	isNumericCapability(capabilityType: CapabilityTypes): boolean {
		return (
			capabilityType === CapabilityTypes.RANGE ||
			capabilityType === CapabilityTypes.GAUGE
		);
	}
	...
	async findByDeviceId(
		...
		try {
			const [lowerBound, upperBound] = this.getIntervalBounds(
				queryDate || new Date(),
				interval,
			); // by doing this , dont need frontend to manually specify date bounds
			const isNumeric = this.isNumericCapability(capabilityType); // if the capability is a numeric type

			const match = {
				// the match for the aggregation pipeline
			...
			const results = await this.model
				.aggregate([
					// should get all data within time frame for a capability of a device
					{ $match: match },
...
export const DeviceTelemetryModelInstance = new DeviceTelemetryModel(
	MongoConnection,

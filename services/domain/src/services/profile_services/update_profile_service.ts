...
export default async function UpdateProfileService(
	changes: UpdatePatch<IProfile>,
	userID: string,
): Promise<ServiceResult<IProfile>> {
	try {
		if (!userID) {
		...
		const [updatedItem, error] = await ProfileModelInstance.update(
			userID,
			changes,
		);
		if (error) {
		...
		if (!updatedItem) {
			throw new Error(
				"Profile update succeeded but no updated item returned",

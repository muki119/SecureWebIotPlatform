...
import { DomainModelInstance, UserDomainModelInstance, UserRoleModelInstance, type IDomain, type IUserDomain, } from "../../models";
...
export default async function CreateDomainService(domainName: string, userID: string): Promise<ServiceResult<{ domain: IDomain, userDomain: IUserDomain }>> {
    try {
        const result = await DomainModelInstance.multiTableTransaction(async (conn) => {
            ...
            const ownerRole = await UserRoleModelInstance.create({ userId: userID, domainId: newDomain.id, role: ROLES.OWNER }, conn) // create a new user-role relationship with the role of owner
            if (!newDomain || !userDomain || !ownerRole) {
                throw new Error("Failed to create domain or user-domain relationship or user-role relationship");
            }
            return { domain: newDomain, userDomain, ownerRole }
        })
        await EventBusInstance.send(STREAMS.DOMAIN_SERVICE.DOMAIN_CREATED, result.ownerRole) // owner role is flat enough to not cause problems or need stringifying
        return [result, null]
    } catch (error) {

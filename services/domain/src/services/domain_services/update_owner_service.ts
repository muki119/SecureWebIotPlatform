...
export default async function UpdateOwnerService(
	ownerId: string,
	newOwnerId: string,
	domainId: string,
	...
	try {
		...
		const ownerPermissions = await UserRoleModelInstance.userPermissions(
			ownerId,
			domainId,
		);
		...
		const newOwnerPermissions = await UserRoleModelInstance.userPermissions(
			newOwnerId,
			domainId,
		);
		...
		throw new Error("Failed to transfer domain ownership", {
			cause: error,
		});
	}
}

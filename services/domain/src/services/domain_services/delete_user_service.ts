...
export default async function DeleteUserService(
	...
	try {
		if (userId === userToDelete) {
			...
			return [
				null,
				new Error("User cannot delete themselves from the domain"),
			];
		}
		const isUserToDeleteMember =
			await UserRoleModelInstance.userPermissions(userToDelete, domainId);
		if (!isUserToDeleteMember) {
			// user to be deleted has to be member
			return [null, new Error("User is not a member of the domain")];
		}
		if (isUserToDeleteMember.isOwner) {
			// user to be deleted cannot be owner - ownership has to be transferred first
			return [
				null,
				new Error(
					"Cannot delete the owner of the domain, ownership must be transferred first",
				),
			];
		}
		const isUserAllowedToDelete =
			await UserRoleModelInstance.userPermissions(userId, domainId);
		if (!isUserAllowedToDelete) {
			// user performing the delete has to be a member
			return [null, new Error("User has no role in the domain")];
		}
		if (!isUserAllowedToDelete.canManageUsers) {
			// user performing the delete has to be admin or owner
			return [
				null,
				new Error("User is not allowed to manage users in this domain"),
			];
		}
		...
		await EventBusInstance.send(
			...
			{ userId: userToDelete, domainId, initiatorId: userId },
		); // send the deleted user id and domain id to the event bus
		return [true, null];
	} catch (error) {

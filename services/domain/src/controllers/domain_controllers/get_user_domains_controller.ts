...
export default async function GetUserDomainsController(
	req: Request,
	res: Response,
	next: NextFunction,
) {
	...
	try {
		...
		const [domains, error] = await GetUserDomainsService(
			userId,
			Number(limit),
			Number(offset),
		);
		if (error) {
			logger.warn(
				...
				"Error in GetUserDomainsController: ",
			);
			return res
				.status(400)
				.json({ message: "Failed to get user domains" })
				.end();
		}

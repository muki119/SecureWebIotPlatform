...
export default async function AddUserController(
	req: Request,
	res: Response,
	next: NextFunction,
) {
	...
	try {
		...
		if (role === ROLES.OWNER) {
			return res
				.status(400)
				.json({ message: "Cannot assign owner role to another user" })
				.end();
		}
		...
		const [_, error] = await AddUserService(
			userId,
			id,
			domainId as string,
			role,
		...
		return res
			.status(200)
			.json({ message: "User successfully added to domain" })
			.end();
	} catch (error) {

...
export class AuthClientRequest {
	...
	static ErrInvalidRefreshToken = new Error(
		"Invalid refresh token, please login again",
	);
	static ErrUnauthorized = new Error(
		"Unauthorized, refresh failed or max retries reached",
	); // for 401s only  -- 400 is a bad request only
	...
	async refresh(): Promise<Result<string>> {
		// should return boolean indicating if its success full
		if (this.refreshPromise) {
			// if theres already a refresh request in progress then wait for it to finish and return the result of that instead of making a new request
			return this.refreshPromise;
		}
		this.refreshPromise = (async () => {
			// if there isnt a request in progress then make a new one
			try {
				...
				if (isAxiosError(error) && error.response) {
					...
					if (error.response.status === 401) {
						// if tokens are invalid or whatnot then we need to force login
				...
				return [
					null,
					new Error("Error refreshing access token", {
						cause: error,
					}),
				];
			} finally {
	...
	protected async performRequest(
		func: (accessToken?: string) => Promise<AxiosResponse>,
		...
		try {
			...
			if (isAxiosError(error)) {
				if (error.response?.status === 401) {
					...
					if (retry) {
						// if theres retries left attempt to retry
						return this.performRequest(
							() =>
								func(
									AuthClientRequest.createAuthHeader(
										newAccessToken,
									),
								),
							retry - 1,
						); // attempt to perform the request again with one less retry
					}
					return [null, AuthClientRequest.ErrUnauthorized];
				} else if (error.response?.status === 500) {
					return [null, AuthClientRequest.ErrServerError];
				...
				if (error.response) {
					// if it ras a response error from the server , axios loves to throw errors for any status code not 200
			...
			return [
				null,
				new Error("Error making post request", { cause: error }),
			];
		}
	}

	static createAuthHeader(accessToken?: string | null) {
	...
	async logout(
		...
		try {
			...
			return [
				null,
				new Error("Error making logout request", { cause: error }),
			];
		}
	}
}
